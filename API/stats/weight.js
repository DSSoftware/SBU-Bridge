...
module.exports = (profile) => {
    ...
    const { catacombs_experience, catacombs, master_mode } = formatLilyDungeons(getDungeons(profile, undefined));
        ...
        senither: {
            ...
            dungeons: {
                ...
                classes: {
                    healer: {
                        total:
                            senither.dungeons.classes.healer.weight + senither.dungeons.classes.healer.weight_overflow,
                    ...
                    archer: {
                        total:
                            senither.dungeons.classes.archer.weight + senither.dungeons.classes.archer.weight_overflow,
...
function formatLilySkills(skills) {
    ...
    const skillSort = ['enchanting', 'taming', 'alchemy', 'mining', 'farming', 'foraging', 'combat', 'fishing', 'hunting'];
    const whitelistedSkills = Object.keys(skills).filter(
        (skill) => !['runecrafting', 'social', 'carpentry', 'hunting'].includes(skill)
    );
...
function formatLilyDungeons(dungeons) {
    ...
    for (const floor of Object.keys(dungeons.catacombs?.master_mode_floors || {})) {
        Object.assign(master_mode, {
            [floor.split('_')[1]]: dungeons.catacombs.master_mode_floors[floor].completions
        });
    }
