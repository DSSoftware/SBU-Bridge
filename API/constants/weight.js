    ...
    const weight = {
        ...
        dungeons: {
            catacombs: calculateSenitherWeight(
                'catacombs',
                calcSkill('dungeoneering', profile.dungeons?.dungeon_types?.catacombs?.experience || 0)
                    .levelWithProgress,
            ...
            classes: {
                healer: calculateSenitherWeight(
                    'healer',
                    calcSkill('dungeoneering', profile.dungeons?.player_classes?.healer?.experience || 0)
                        .levelWithProgress,
                    profile.dungeons?.player_classes?.healer?.experience || 0
                ),
                mage: calculateSenitherWeight(
                    'mage',
                    calcSkill('dungeoneering', profile.dungeons?.player_classes?.mage?.experience || 0)
                        .levelWithProgress,
                    profile.dungeons?.player_classes?.mage?.experience || 0
                ),
                berserk: calculateSenitherWeight(
                    'berserk',
                    calcSkill('dungeoneering', profile.dungeons?.player_classes?.berserk?.experience || 0)
                        .levelWithProgress,
                    profile.dungeons?.player_classes?.berserk?.experience || 0
                ),
                archer: calculateSenitherWeight(
                    'archer',
                    calcSkill('dungeoneering', profile.dungeons?.player_classes?.archer?.experience || 0)
                        .levelWithProgress,
                    profile.dungeons?.player_classes?.archer?.experience || 0
                ),
                tank: calculateSenitherWeight(
                    'tank',
                    calcSkill('dungeoneering', profile.dungeons?.player_classes?.tank?.experience || 0)
                        .levelWithProgress,
