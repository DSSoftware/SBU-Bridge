...
async function infoMessage(message) {
  return console.log(
    chalk.inverse(`[${await getCurrentTime()}] Info >`) + " " + message,
  );
}
...
async function replicationInfo(message) {
  return console.log(
    chalk.bgBlueBright.black(`[${await getCurrentTime()}] Replication >`) +
      " " +
...
async function minecraftMessage(message) {
  return console.log(
    chalk.bgGreenBright.black(`[${await getCurrentTime()}] Minecraft >`) +
      " " +
...
async function webMessage(message) {
  return console.log(
    chalk.bgCyan.black(`[${await getCurrentTime()}] Web >`) +
      " " +
      chalk.cyan(message),
  );
}
